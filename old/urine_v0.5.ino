...
char checkCup(){ // 'N' cup, 'U'ser has cup, 'F'ull cup, 'L'ab has cup
  ...
  if(current_position == 'B'){ // if passthrough is facing the bathroom...
    ...
    if(cup == false){  // lab has the cup if it isn't there
      ...
      if(cup_state == 'L'){ // if there is a cup now, but wasn't last time, it is new
        cup_state = 'N';
      }else if(cup_state == 'N'){  // if the cup is there, and it is new, then it is still new
        cup_state = 'N';
      }else{  // if the cup is there, but it isn't new, then it is full (or needs to be replaced)
        cup_state = 'F';
      }
    }
